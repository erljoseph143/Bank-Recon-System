...
@section('crumb')
    ...
    <li class='breadcrumb-item active'><a href="{{ route('bsmonths', [$id, $account, $code]) }}">Bank Statement in a month list</a></li>
    <li class='breadcrumb-item active'>{{ $pagetitle }}</li>
@endsection

@section('table-nav')
    <button onclick="location.href='{{ route('bsview', [$id, $account, $code, $year, $month, 'p'=>'all']) }}';" type="button" class="btn @if($template == 'all') btn-primary @else btn-default @endif btn-custom waves-effect w-md waves-light m-b-5">Active &#40;<span class="all-count">{{ $countall }}</span>&#41;</button>
    <button onclick="location.href='{{ route('bsview', [$id, $account, $code, $year, $month, 'p'=>'trash']) }}';" type="button" class="btn @if($template == 'trash') btn-primary @else btn-default @endif btn-custom waves-effect w-md waves-light m-b-5">Trash &#40;<span class="trash-count">{{ $counttrash }}</span>&#41;</button>
@endsection
    ...
    <div class="col-sm-12">
        <div class="portlet">
            <div class="portlet-heading portlet-default">
                ...
                <p class="text-muted font-10 m-b-10 inline">Bank statements under {{ $bu->bname }} with bank account details {{ $accountdata->bank . '-'.$accountdata->accountno.'-'.$accountdata->accountname }}</p>
            ...
            <div class="panel-collapse collapse show">
                <div class="portlet-body">
                    @yield('table-nav')
                    <table id="bs" class="table m-b-0 table-hover table-bordered toggle-arrow-tiny checks" data-action="{{ route('bsviewajax', ['code' => $code, 'bu' => $id,'year' => $year, 'month' => $month, 'page' => $template]) }}">
                        <thead>
                        <tr>
