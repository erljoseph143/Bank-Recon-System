...
@section('content')
    <table id="account" class="table m-b-0 table-bordered toggle-arrow-tiny" data-page-size="8" data-limit-navigation="3">
        ...
        @foreach($bs as $key => $b)
            <tr>
                <td>{{ $b->bank . '-' . $b->accountno . '-' . $b->accountname }}</td>
                <td class="actions">
                    <a href="{{ route('bsmonths',[$id, $b->id, $b->bank_account_no]) }}" class="on-default view-checks" title="view"><i class="fa fa-television font-18"></i></a>
                </td>
            </tr>
        @endforeach
        </tbody>
